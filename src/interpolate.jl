...
function linear_interp(xs::AbstractArray{T,1}, ys::AbstractArray{T,1}; bc::T=NaN) where T<:Float64
    function f(x)
        ...
        elseif x >= last(xs)
            ...
            # TODO: pen and paper figure out the below (top is correct)
            # TODO: subtracting ys[i1] - ys[i0] is unecessary cancellation
            # return (ys[i0] * (xs[i0] - x) + ys[i1] * (x - xs[i0])) / (xs[i1] - xs[i0])
        end
    end
    return f
end
