function precompute_quantities_gpu!(disk::DiskParams{T1}, ϕc::CuArray{T2,2},
    ...
    @cusync @captured @cuda threads=threads1 blocks=blocks1 precompute_quantities_gpu!(ϕc, θc, μs, wts, z_rot, ax_codes,
                                                                                       Nϕ, Nθ_max, Nsubgrid, Nθ, R_x, O⃗,
                                                                                       ρs, A, B, C, v0, u1, u2)
...
function precompute_quantities_gpu!(ϕc, θc, μs, wts, z_rot, ax_codes, Nϕ, Nθ_max,
                                    Nsubgrid, Nθ, R_x, O⃗, ρs, A, B, C, v0, u1, u2)
    ...
    for t in idx:sdx:num_tiles
        ...
        for ti in i:i+k-1
            ...
            for tj in j:j+k-1
                ...
                x, y, z = sphere_to_cart_gpu(ρs, ϕc_sub, θc_sub)
                ...
                rp = 2π * ρs * CUDA.cos(ϕc_sub) / rotation_period_gpu(ϕc_sub, A, B, C)
                ...
                dA = calc_dA_gpu(ρs, ϕc_sub, dϕ, dθ)
                dA *= μ_sub
                dA_sum += dA
    ...
    for i in idx:sdx:CUDA.length(μs)
        ...
        for j in 1:Nsubgrid
            ...
            for k in 1:Nsubgrid
                ...
                # set magnitude by differential rotation
                rp = 2π * ρs * CUDA.cos(ϕsub) / rotation_period_gpu(ϕsub, A, B, C)
