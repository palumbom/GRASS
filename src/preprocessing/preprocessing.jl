...
function calc_width_at_depth(wavs::AA{T,1}, spec::AA{T,1}; len::Integer=100) where T<:Float64
    # check that the length of arrays match
    @assert length(wavs) == length(spec)
...
function calc_width_at_depth(wavs::AA{T,2}, spec::AA{T,2}; center::T=NaN,
    ...
    return calc_width_at_depth(wavs[:,1], spec[:,1], len=len, center=center)
end

function calc_bisector_at_depth(wavs::AA{T,1}, spec::AA{T,1}; len::Integer=100) where T<:Real
    # check that the length of arrays match
    @assert length(wavs) == length(spec)
