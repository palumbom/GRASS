...
function calc_width_at_depth(wavs::AA{T,1}, spec::AA{T,1};
    ...
    lwavind = searchsortednearest(wavs, 5434.15)
    rwavind = searchsortednearest(wavs, 5434.9)
    ...
    lwing_wavs, lwing_flux = fit_line_wings(newwavs, newspec, side="left")
    rwing_wavs, rwing_flux = fit_line_wings(newwavs, newspec, side="right")
...
function calc_width_at_depth(wavs::AA{T,2}, spec::AA{T,2};
                             len::Integer=100) where T<:Real
    out = map(calc_width_at_depth, eachcol(wavs), eachcol(spec))
    return cat([x[1] for x in out]..., dims=2), cat([x[2] for x in out]..., dims=2)
end

function fit_line_wings(wavs, spec; side="left")
    # cut out the middle
    botind = argmin(spec)
    if side == "left"
    ...
    # perform the fit
    p0 = [-0.125, 5434.5232, 0.03, 0.03, 1.0]
