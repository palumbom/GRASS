...
function synthesize_spectra_eclipse(spec::SpecParams{T}, disk::DiskParamsEclipse{T}, obs_long, obs_lat, alt, wavelength, time_stamps, band, neid_ext_coeff;
    ...
    if use_gpu
        return synth_Eclipse_gpu(spec, disk, seed_rng, verbose, precision, skip_times, obs_long, obs_lat, alt, time_stamps, band, wavelength, neid_ext_coeff)
    else
        return synth_Eclipse_cpu(spec, disk, seed_rng, verbose, skip_times, obs_long, obs_lat, alt, time_stamps, band, wavelength, neid_ext_coeff)
    end
end

function synth_Eclipse_cpu(spec::SpecParams{T}, disk::DiskParamsEclipse{T}, seed_rng::Bool,
                   verbose::Bool, skip_times::BitVector, obs_long, obs_lat, alt, time_stamps, band, wavelength, neid_ext_coeff) where T<:AF
    ...
    for (idx, file) in enumerate(templates)
        ...
        disk_sim_eclipse(spec_temp, disk, soldata, wsp, mem, prof, flux, tloop, tloop_init, templates, idx, obs_long, obs_lat, alt, time_stamps, band, wavelength, neid_ext_coeff,
...
function synth_Eclipse_gpu(spec::SpecParams{T}, disk::DiskParamsEclipse{T}, seed_rng::Bool,
                   verbose::Bool, precision::DataType, skip_times::BitVector, obs_long, obs_lat, alt, time_stamps, band, wavelength, neid_ext_coeff) where T<:AF
