...
function neid_all_lines(neid_time, granulation_status)

    file = DataFrame(CSV.File("data/NEID_extinction_coefficient.csv"))

    granulation = granulation_status
    ...
    for i in eachindex(lp.λrest) 
        ...
        lambdas_cpu, outspec_cpu = GRASS.synthesize_spectra_eclipse(spec, disk, obs_long, obs_lat, alt, lines ./ 10.0, time_stamps, "Optical", neid_ext_coeff, ext_toggle = false, verbose=true, use_gpu=false)
...
# rv, rv_error, λrest = neid_all_lines(neid_october, true)
# @save "neid_all_lines_rv_regular_Kostogryz_LD_SSD.jld2"
# jldopen("neid_all_lines_rv_regular_Kostogryz_LD_SSD.jld2", "a+") do file
#     file["name"] = λrest 
#     file["rv"] = rv 
#     file["rv_error"] = rv_error 
# end

rv, rv_error, λrest = neid_all_lines(neid_october, false)
@save "neid_all_lines_rv_off_Kostogryz_LD_SSD.jld2"
jldopen("neid_all_lines_rv_off_Kostogryz_LD_SSD.jld2", "a+") do file
    file["name"] = λrest 
    file["rv"] = rv 
    file["rv_error"] = rv_error 
end
