...
function neid_all_lines_gpu(neid_time, granulation_status, LD_type, ext_toggle)
    ...
    N = 2000
    Nt = length(time_stamps)
    disk = GRASS.DiskParamsEclipse(N=N, Nt=Nt, Nsubgrid=50)
...
function neid_all_lines_gpu_fine_time(neid_time, granulation_status, LD_type, ext_toggle)
    # convert from utc to et as needed by SPICE
    time_stamps = utc2et.(neid_time)
    ...
    orders = [57, 57, 60, 60, 60, 60, 61, 61, 61, 61, 61, 61, 64, 64, 74, 74, 75, 75, 75, 75, 77, 77]
    pixels = [3021:3086, 3062:3128, 2227:2287, 2355:2416, 2464:2526, 2564:2626, 2702:2764, 2738:2801, 2881:2944, 3003:3067, 3057:3093, 3070:3134, 2303:2362, 2529:2589, 3563:3622, 3765:3824, 378:425, 414:461, 478:526, 674:722, 734:781, 802:849]
    for i in 3:3#eachindex(lp.λrest) 
        println("\t>>> Template: " * string(splitdir(lfile[i])[2]))
        ...
        ext_coeff_file = DataFrame(CSV.File("data/NEID_ext_coeff_SSD_fine_time.csv"))
        coeff1 = ext_coeff_file[ext_coeff_file[!, "Wavelength"] .== λrest[i], "ext"][1]
        ...
        for i in 1:num_blocks
            ...
            # len = length(unbinned_flux[:, start_col:end_col][:, 1])
            # outspec_cpu[:, i] = ((unbinned_flux[:, start_col:end_col][:, 1][1:len].*0.25) .+ (unbinned_flux[:, start_col:end_col][:, 2][1:len].*0.25)
            #                     .+ (unbinned_flux[:, start_col:end_col][:, 3][1:len].*0.25) .+ (unbinned_flux[:, start_col:end_col][:, 4][1:len].*(1/6))) ./ (11/12)
        end      
    ...
    return rv, rv_error, λrest, rv_var, rv_error_var, rv_var_lsf, rv_error_var_lsf, rv_lsf, rv_error_lsf, brightness
end
...
function neid_october_eclipse_var_off_gpu(LD_type, ext_toggle)
    ...
    rv, rv_error, λrest = deepcopy(neid_all_lines_gpu(time_stamps, false, LD_type, ext_toggle))
...
function neid_october_eclipse_var_on_gpu(LD_type, ext_toggle)
    ...
    rv, rv_error, λrest, rv_var, rv_error_var, rv_var_lsf, rv_error_var_lsf, rv_lsf, rv_error_lsf = deepcopy(neid_all_lines_gpu(neid_october, true, LD_type, ext_toggle))
...
function neid_october_eclipse_var_off_gpu_fine_time(LD_type, ext_toggle)
    ...
    df = CSV.File("data/NEID_October_fine_time.csv") |> DataFrame

    # Display the DataFrame
    neid_october = string.(df[!, "FineTime"])

    rv, rv_error, λrest, rv_var, rv_error_var, rv_var_lsf, rv_error_var_lsf, rv_lsf, rv_error_lsf, brightness = deepcopy(neid_all_lines_gpu_fine_time(neid_october, false, LD_type, ext_toggle))
    @save "neid_all_lines_rv_off_$(LD_type)_gpu_fine_time_KTS.jld2"
    jldopen("neid_all_lines_rv_off_$(LD_type)_gpu_fine_time_KTS.jld2", "a+") do file
        file["name"] = deepcopy(λrest)
        file["rv"] = deepcopy(rv) 
        file["rv_error"] = deepcopy(rv_error)
        file["rv_var"] = deepcopy(rv_var) 
        ...
        file["rv_var_lsf"] = deepcopy(rv_var_lsf) 
        file["rv_error_var_lsf"] = deepcopy(rv_error_var_lsf)
        file["brightness"] = deepcopy(brightness)
    end
end
