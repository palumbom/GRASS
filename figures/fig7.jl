...
if plot
    ...
    for i in eachindex(axs)
        # histogram for short dead time
        axs[i].hist(rms_shortdead[i,:], density=true, histtype="stepfilled", fc="k", ec="k", lw=2, alpha=0.5, label="Short wait")
        axs[i].hist(rms_shortdead[i,:], density=true, histtype="step", ec="k", lw=1.0, alpha=0.8)

        # histogram for long wait time
        axs[i].hist(rms_largedead[i,:], density=true, histtype="stepfilled", fc="tab:blue", ec="tab:blue", lw=1.5, alpha=0.5, label="Long wait")
        axs[i].hist(rms_largedead[i,:], density=true, histtype="step", ec="tab:blue", lw=1.5, alpha=0.8)
        ...
        # report two-sample KS test
        @show(ApproximateTwoSampleKSTest(rms_shortdead[i,:], rms_largedead[i,:]))
    end
    ...
    for i in eachindex(axs)
        # annotate axes and set labels, etc.
        axs[i].annotate(L"N_{\rm obs} =\ " * latexstring(N_obs[i]), xy=xy, bbox=bbox)
        axs[i].set_xlabel(L"{\rm RMS\ RV\ (m s}^{-1})")
        axs[i].set_ylabel(L"{\rm Probability\ density}")
        # axs[i].set_xlim(round(minimum(first.(xlims)), digits=0) - 0.25,
        #                 round(maximum(last.(xlims)), digits=0) + 0.25)
        # axs[i].set_ylim(minimum(first.(ylims)), maximum(last.(ylims)))
