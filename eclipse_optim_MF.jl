...
function projected_RV_gpu(params)
    for i in 1:1#eachindex(lp.λrest) 
        ...
        for t in 1:disk.Nt
            ...
            GRASS.calc_eclipse_quantities_gpu!(time_stamps[t], t, obs_long, obs_lat, alt, lines, 
                                        1.0, neid_ext_coeff, disk, gpu_allocs, NaN, NaN, NaN, NaN, parse(Float64, string(optim_list[2])), parse(Float64, string(optim_list[3])), parse(Float64, string(optim_list[4])), params[1], params[2])
