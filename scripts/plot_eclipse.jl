...
# obs_long = -17.889242  
# obs_alt = 2.359
# time_stamps = range(utc2et("2014-01-05T15:00:00.0"), utc2et.("2014-01-06T01:30:00.0"), step = 210.0)
...
outdir = joinpath(pwd(), "2026Transit")
if !isdir(outdir)
    mkdir(outdir)
end
...
for t in eachindex(time_stamps)
    ...
    GRASS.calc_eclipse_quantities_gpu!(time_stamps[t], obs_long, obs_lat, obs_alt, lines, 
                                       LD_type, 0.0, neid_ext_coeff[1], disk, gpu_allocs)
