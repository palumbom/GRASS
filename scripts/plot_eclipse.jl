...
for t in length(time_stamps):length(time_stamps)#eachindex(time_stamps)

    # GRASS.calc_eclipse_quantities_gpu!(time_stamps[t], obs_long, obs_lat, obs_alt, lines, 
    #                                LD_type, 1.0, ext_coeff_array[8], disk, gpu_allocs, 1.0)
    optim_list = df_optim_CB_MF[df_optim_CB_MF.Column1 .== splitext(string(splitdir(lfile[9])[2]))[1], :][1, :]
    GRASS.calc_eclipse_quantities_gpu!(time_stamps[t], obs_long, obs_lat, obs_alt, lines, ext_coeff_array[8], disk, gpu_allocs,
    ...
    mf = Array(gpu_allocs.mf)
    ld = Array(gpu_allocs.ld)[:,:,1]
    vrot = Array(gpu_allocs.projected_v)
    ...
    ax1.set_xlabel("Azimuth", fontsize = 18)
    ax1.set_ylabel("Altitude", fontsize = 18)

    # make colormap for data
    cmap = plt.cm.seismic
    # norm = mpl.colors.Normalize(vmin=0.0, vmax=1.0)
    # norm = mpl.colors.Normalize(vmin=-2400, vmax=2400)
    norm = mpl.colors.Normalize(vmin=minimum(mf), vmax=maximum(mf))
    smap = plt.cm.ScalarMappable(cmap=cmap, norm=norm)

    # add a colorbar 
    cbar = fig.colorbar(smap, ax=ax1, orientation="vertical")
    # cbar.set_label(label="Relative Limb Darkening", size = 14)
    ...
    ax1.text(x=0.02, y=0.98, s=split_time[1] * ":" * split_time[2] * " UTC", transform=ax1.transAxes,
