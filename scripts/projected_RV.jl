...
function projected_RV(time, filename, LD_type)
    #convert from utc to et as needed by SPICE
    time_stamps = utc2et.(time)

    variable_names = ["dA_total_proj_mean", "mean_weight_v_no_cb", "mean_weight_v_earth_orb", "zenith_mean", "dA_total_proj", "idx1", "idx3", "mu_grid", "z_rot_sub", "N"]
    ...
    for i in eachindex(lp.λrest) 
        ...
        lines = [λrest[i]]
        neid_ext_coeff = extinction_coeff[:, names(extinction_coeff)[i+1]]
        ...
        for t in 1:disk.Nt
            mean_intensity = GRASS.eclipse_compute_intensity(disk, lines, neid_ext_coeff, LD_type, idx1[t], idx3[t],
                                                                        ...
                                                                        z_rot_sub[t], dA_total_proj[t], wsp.ld, wsp.z_rot, zenith_mean[:, :, t], false)
    ...
    @save "$(filename)_KSSD.jld2"
    jldopen("$(filename)_KSSD.jld2", "a+") do file
...
projected_RV(exp_meter_time[6001:-1], "neid_october_exp_meter_N_50_7000", "KSSD")