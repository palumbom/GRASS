...
function neid_all_lines(neid_time, granulation_status, filename, LD_type, ext_toggle)
    ...
    rvs_cpu = Vector{Float64}(undef,size(time_stamps)...)
    sigs_cpu = Vector{Float64}(undef,size(time_stamps)...)
    resolution = 7e5
    # loop over lines
    for i in eachindex(lp.λrest) 
        ...
        for t in 1:size(flux_sim, 2)
            ...
            # measure velocities
            v_grid_cpu, ccf_cpu, ccf_var_out = deepcopy(GRASS.calc_ccf(wavs_sim_itp, flux_sim_itp, filter(!=(0), NEID_variance[i][:,t]), lines, depths, 11e4))
            rvs_cpu[t], sigs_cpu[t] = deepcopy(GRASS.calc_rvs_from_ccf(v_grid_cpu, ccf_cpu, ccf_var_out))
        end
...
function gpu_N_convergence(neid_time, granulation_status, filename, LD_type, ext_toggle)
    ...
    for j in N_range
        ...
        rv[count] = deepcopy(rvs_cpu)
        rv_error[count] = deepcopy(sigs_cpu)
        count = count + 1
    end
    return rv, rv_error, N_range
end
...
function neid_october_eclipse_var_off(LD_type, ext_toggle)
    ...
    rv, rv_error, λrest = deepcopy(neid_all_lines(neid_october, false, "neid_october_N_50", LD_type, ext_toggle))
...
function neid_october_eclipse_var_on(LD_type, ext_toggle)
    ...
    rv, rv_error, λrest = deepcopy(neid_all_lines(neid_october, true, "neid_october_N_50", LD_type, ext_toggle))
...
function neid_october_eclipse_var_off_gpu(LD_type, ext_toggle)
    ...
    rv, rv_error, N_range = deepcopy(gpu_N_convergence(neid_october, false, "neid_october_N_50", LD_type, ext_toggle))
    @save "N_convergence.jld2"
    jldopen("N_convergence.jld2", "a+") do file
        file["N_range"] = deepcopy(N_range) 
        file["rv"] = deepcopy(rv) 
        file["rv_error"] = deepcopy(rv_error)
    end

    # rv, rv_error, λrest = deepcopy(neid_all_lines_gpu(neid_october, false, "neid_october_N_50", LD_type, ext_toggle))
...
function neid_october_eclipse_var_on_gpu(LD_type, ext_toggle)
    ...
    rv, rv_error, λrest = deepcopy(neid_all_lines_gpu(neid_october, true, "neid_october_N_50", LD_type, ext_toggle))
...
function neid_october_nxt_day_var_off(LD_type, ext_toggle)
    ...
    rv, rv_error, λrest = deepcopy(neid_all_lines(neid_1015, false, "neid_october_N_50_nxt_day", LD_type, ext_toggle))
...
function neid_october_nxt_day_var_on(LD_type, ext_toggle)
    ...
    rv, rv_error, λrest = deepcopy(neid_all_lines(neid_1015, true, "neid_october_N_50_nxt_day", LD_type, ext_toggle))
